...
// -------------------------------------------------------------
// Hardcoded paths (fallback). You can override with argv[1].
// -------------------------------------------------------------
// static const char* kFallbackPath = "C:\\datasets\\MRI_DICOM\\IMG-0001-00001_fromSimens.dcm";
static const char* kFallbackPath = "C:\\datasets\\MRI_raw\\FastMRI\\brain_multicoil\\file_brain_AXFLAIR_200_6002452.h5";
// static const char* kFallbackPath = "C:\\datasets\\MRI_raw\\from mridata_dot_org\\52c2fd53-d233-4444-8bfd-7c454240d314.h5";
...
// Pick input path: argv[1] (if present) or fallback
static QString pickInputPath(int argc, char** argv, simplelog::Logger& log) {
    ...
    log_and_print(log, std::string("[DBG][Main] Using fallback path: ") + fb.toStdString());
    return fb;
}
...
int main(int argc, char** argv)
{
    ...
    try {
        ...
        MainWindow w;
        AppController controller(&w);   // No need to pass Logger; Qt handler captures logs
        ...
        if (!fi.exists()) {
            log_and_print(log, "[WRN][Main] Input file does not exist. Continuing anyway; loaders will report errors.");
        }
