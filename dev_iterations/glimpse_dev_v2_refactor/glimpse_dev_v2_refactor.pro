...
# NOTE: You pointed VCPKG_ROOT to the *triplet install*. That's OK for debug-only.
...
DEFINES += HAVE_ISMRMRD          # <--- enable ISMRMRD code paths
...
# ---------------- OpenMP ----------------
# MSVC (Qt msvc2022_64): compiler flag only — do NOT add to QMAKE_LFLAGS
win32:msvc {
    # Try modern LLVM OpenMP first. If your toolset doesn’t support it,
    # switch this to /openmp (classic) instead.
    QMAKE_CXXFLAGS += /openmp:llvm
    DEFINES += OMP_ENABLED=1
    message("[DBG] OpenMP: MSVC /openmp:llvm (compiler only)")
}

# If you still see warnings, comment the line above and use classic runtime:
# win32:msvc {
#     QMAKE_CXXFLAGS += /openmp
#     DEFINES += OMP_ENABLED=1
#     message("[DBG] OpenMP: MSVC /openmp (classic)")
# }
...
# Linux/macOS (Clang/GCC). (macOS may need `brew install libomp`)
unix:!win32 {
    QMAKE_CXXFLAGS += -fopenmp
    QMAKE_LFLAGS   += -fopenmp
    DEFINES += OMP_ENABLED=1
...
# ISMRMRD depends on HDF5 and pugixml. Keep your HDF5 libs as-is for now.
LIBS += -lismrmrd -lpugixml     # <--- add ISMRMRD + pugixml
LIBS += -lhdf5_D -lhdf5_hl_D -lhdf5_cpp_D -lhdf5_hl_cpp_D
LIBS += -lzlibd -laec

# DCMTK image + core libs (needed for DicomImage)
...
# NOTE: If you ever put OpenMP pragmas in .cu host code, add:
...
# If you add dcmjpeg/charls/jpeg/png/tiff above, copy their DLLs too if needed.
