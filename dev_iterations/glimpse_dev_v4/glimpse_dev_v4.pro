...
HEADERS += \
    ...
    view/mainwindow.hpp \
    src/image_utils.hpp
# NOTE: do NOT add io_ismrmrd_api.h here if you’re using the one from the DLL’s include folder.

# Add back the CPU ISMRMRD reader (app-side)
SOURCES +=
HEADERS +=
...
# Copy your engine DLL next to the app (updated name + var; guarded)
win32:exists($$MRI_DLL_DIR) {
    QMAKE_POST_LINK += $$quote(cmd /c xcopy /Y /I /Q "$$MRI_DLL_DIR\\mri_engine_v_1_1.dll" "$$DEST_DLL_DIR\\")
}

# vcpkg runtime DLLs still needed by your app
